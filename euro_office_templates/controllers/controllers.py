...
class Onlyoffice_Inherited_Connector(Onlyoffice_Connector):
    ...
    def get_template_content(self, path):
        try:
            file_content = request.env["euro_office.odoo.demo.templates"].get_template_content(path.replace("_", "/"))
    ...
    @http.route("/euro_office/template/editor", auth="user", methods=["POST"], type="json", csrf=False)
    def override_render_editor(self, attachment_id, access_token=None):
        ...
        hasAccess = http.request.env.user.has_group("euro_office_templates.group_euro_office_templates_admin")
...
class OnlyofficeTemplate_Connector(http.Controller):
    ...
    def main(self, template_id, record_ids):
        ...
        try:
            ...
            if len(templates) == 1:
                ...
                response = euro_office_request(url=quote(url, safe="/:?=&"), method="get")
                if response.status_code == 200:
                    ...
                    logger.info("GET /euro_office/template/fill - returning single PDF: %s", filename)
                ...
                logger.info("GET /euro_office/template/fill - creating ZIP with %s files", len(templates))
                stream = io.BytesIO()
                with zipfile.ZipFile(stream, "w", zipfile.ZIP_DEFLATED) as archive:
                    for filename, url in templates.items():
                        response = euro_office_request(url=url, method="get")
                        if response.status_code == 200:
                ...
                filename = f"euro_office-templates-{datetime.now().strftime('%Y_%m_%d_%H_%M')}.zip"
                headers = [
                ...
                logger.info("GET /euro_office/template/fill - returning ZIP: %s", filename)
    ...
    def fill_template(self, oo_security_token, record_ids, template_id):
        ...
        docbuilder_callback_url = f"{odoo_url}euro_office/template/callback/docbuilder/fill_template?oo_security_token={oo_security_token}&record_ids={record_ids}&template_id={template_id}"  # noqa: E501
        ...
        try:
            if jwt_secret:
                docbuilder_response = euro_office_request(
                    url=docbuilder_url,
                    method="post",
                    opts={
    ...
    @http.route("/euro_office/template/callback/docbuilder/fill_template", auth="public")
    def docbuilder_fill_template(self, oo_security_token, record_ids, template_id):
        logger.info(
            "GET /euro_office/template/callback/docbuilder/fill_template - template: %s, records: %s",
            template_id,
            record_ids,
        )
        ...
        template = self.get_record("euro_office.odoo.templates", template_id, user)
        if not template:
        ...
        try:
            ...
            logger.info(
                "GET /euro_office/template/callback/docbuilder/fill_template - processing %s records", len(record_ids)
            )
            url = f"{config_utils.get_base_or_odoo_url(http.request.env)}euro_office/template/download/{attachment_id}?oo_security_token={oo_security_token}"  # noqa: E501

            docbuilder_content = ""
            docbuilder_script_content = ""
            with file_open("euro_office_templates/controllers/fill_template.docbuilder", "r") as f:
                docbuilder_script_content = f.read()

            keys = self.get_keys(attachment_id, oo_security_token)
            logger.info(
                "GET /euro_office/template/callback/docbuilder/fill_template - got %s keys", len(keys) if keys else 0
            )
            ...
            logger.info("GET /euro_office/template/callback/docbuilder/fill_template - success")
    ...
    def get_keys(self, attachment_id, oo_security_token):
        ...
        docbuilder_headers = {"Content-Type": "application/json", "Accept": "application/json"}
        docbuilder_callback_url = f"{odoo_url}euro_office/template/callback/docbuilder/get_keys?attachment_id={attachment_id}&oo_security_token={oo_security_token}"  # noqa: E501
        docbuilder_payload = {"async": False, "url": docbuilder_callback_url}
        ...
        try:
            ...
            keys_response = euro_office_request(
                url=keys_url,
                method="get",
            )
    ...
    @http.route("/euro_office/template/callback/docbuilder/get_keys", auth="public")
    def docbuilder_get_keys(self, attachment_id, oo_security_token):
        logger.info("GET /euro_office/template/callback/docbuilder/get_keys - attachment: %s", attachment_id)
        ...
        with file_open("euro_office_templates/controllers/get_keys.docbuilder", "r") as f:
        ...
        logger.info("GET /euro_office/template/callback/docbuilder/get_keys - success")
        return request.make_response(docbuilder_content, headers)

    @http.route("/euro_office/template/download/<int:attachment_id>", auth="public")
    def download(self, attachment_id, oo_security_token):
        logger.info("GET /euro_office/template/download - attachment: %s", attachment_id)
        if not attachment_id or not oo_security_token:
            logger.warning("attachment_id or oo_security_token not found")
            return request.not_found()
        ...
        if attachment:
            ...
            logger.info("GET /euro_office/template/download - success")
            return request.make_response(content, headers)
        else:
    ...
    @http.route("/euro_office/template/documents/check", auth="user", type="json")
    ...
    @http.route("/euro_office/template/documents/folders", auth="user", type="json")
    ...
    @http.route("/euro_office/template/documents/save", auth="user", type="json")
