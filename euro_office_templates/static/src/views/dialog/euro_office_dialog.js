...
export class TemplateDialog extends Component {
  setup() {
    ...
    onWillStart(async () => {
      ...
      const views = await this.viewService.loadViews({
        ...
        resModel: "euro_office.odoo.templates",
        views: [[false, "search"]],
      })
  ...
  async checkDocumentsModule() {
    try {
      this.state.documentsAvailable = await this.rpc("/euro_office/template/documents/check", {})
    } catch {
  ...
  async fetchTemplates(offset = 0) {
    ...
    const length = await this.orm.searchCount("euro_office.odoo.templates", domain, { context })
    if (!length) {
  ...
  addToDocuments() {
    ...
    this.env.services.dialog.add(FolderSelectionDialog, {
      onFolderSelected: async (folderId) => {
        ...
        this.env.services.ui.block()
        try {
          const result = await this.rpc("/euro_office/template/documents/save", {
            template_id: templateId,
            record_ids: resId,
            folder_id: folderId,
          })
...
TemplateDialog.template = "euro_office_templates.TemplateDialog"
