...
import { ExportData } from "./euro_office_editor_export_data"
...
class TemplateEditor extends Component {
  setup() {
    ...
    useBus(this.env.bus, "euro_office-template-create-form", (field) => this.createForm(field.detail))
    ...
    onMounted(async () => {
      try {
        ...
        await this.orm.call("euro_office.odoo.templates", "update_relationship", [id, template_model_model])

        const response = await this.rpc("/euro_office/template/editor", { attachment_id: attachment_id })
                ...
                window.connector.executeMethod("GetCurrentContentControlPr", [], (obj) => {
                  ...
                  if (formKey !== this.lastFormKey) {
                    ...
                    if (formKey) {
                      ...
                      this.env.bus.trigger("euro_office-template-highlight-field", fieldId)
                    } else {
                      this.env.bus.trigger("euro_office-template-highlight-field", null)
                    }
                  }
                })
              })
  ...
  showNoLicenseNotification() {
    if (!this.state.hasLicense && !this.noLicenseNotified) {
      ...
      this.notificationService.add(
        _t(
          "Note: The Euro-Office Automation API is not activated in your instance, so automatic insertion of predefined keys from Odoo into the Euro-Office editor isn't available. You can manually create the field and paste the key from your clipboard.",
        ),
        {
          type: "warning",
          sticky: true,
...
TemplateEditor.template = "euro_office_templates.TemplateEditor"

registry.category("actions").add("euro_office_template_editor", TemplateEditor)
