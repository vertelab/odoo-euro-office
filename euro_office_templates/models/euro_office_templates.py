...
from odoo.addons.euro_office_templates.utils import pdf_utils

logger = logging.getLogger(__name__)


class OnlyOfficeTemplate(models.Model):
    _name = "euro_office.odoo.templates"
    ...
    def _convert_to_form(self, attachment):
        ...
        payload = {
            "url": f"{odoo_url}euro_office/template/download/{attachment.id}?oo_security_token={oo_security_token}",
            "key": key,
            "filetype": "pdf",
            "outputtype": "pdf",
            "pdf": {
    ...
    def open_template_editor(self):
        """
        Open Euro-Office template editor for this record
        """
        self.ensure_one()
        return {
            "type": "ir.actions.client",
            "tag": "euro_office_template_editor",
            "target": "current",
            "params": {
    ...
    def update_relationship(self, template_model_id, model):
        ...
        record = self.sudo().env["euro_office.odoo.templates"].browse(template_model_id)
        if not record:
            return
        ...
        for template in self:
            if not template.report_id:
                report = self.env["ir.actions.report"].create(
                    {
                        "name": f"{template.name} Print (Euro-Office)",
                        "report_type": "euro_office-pdf",
                        "report_name": template.name,
                        "euro_office_template_id": template.id,
