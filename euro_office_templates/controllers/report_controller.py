...
class ReportController(ReportController):
    ...
    def report_routes(self, reportname, docids=None, converter=None, **data):
        if converter == "euro_office-pdf":
            ...
            pdf = report.with_context(**context)._render_euro_office_pdf(reportname, docids, data=data)[0]
            pdfhttpheaders = [
                (
                    "Content-Type",
                    "application/pdf",
    ...
    def report_download(self, data, context=None, token=None, readonly=True):
        ...
        if report_type == "euro_office-pdf":
            try:
                reportname = url.split("/report/euro_office-pdf/")[1].split("?")[0]
                docids = None
                if "/" in reportname:
                ...
                if docids:
                    ...
                    response = self.report_routes(
                        reportname, docids=docids, converter="euro_office-pdf", context=context
                    )
                else:
                    # Particular report:
                    ...
                    response = self.report_routes(reportname, converter="euro_office-pdf", context=context, **data)
