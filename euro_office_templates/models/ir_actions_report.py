...
from odoo.addons.euro_office.controllers.controllers import euro_office_request
from odoo.addons.euro_office.utils import config_utils, jwt_utils, url_utils
...
class IrActionsReport(models.Model):
    ...
    report_type = fields.Selection(
        selection_add=[("euro_office-pdf", "Euro-Office PDF")],
        ondelete={"euro_office-pdf": "set default"},
    )

    euro_office_template_id = fields.Many2one("euro_office.odoo.templates")
    ...
    @api.onchange("euro_office_template_id")
    def onchange_euro_office_template_id(self):
        if self.report_type in ["euro_office-pdf"]:
            self.model = self.euro_office_template_id.template_model_model
            self.report_name = self.euro_office_template_id.name
        else:
            self.model = False
    ...
    def _render_euro_office_pdf_prepare_streams(self, report_ref, data, res_ids=None):
        if not data:
            data = {}
        data.setdefault("report_type", "euro_office-pdf")
        ...
        # Call 'euro_office' to generate the missing streams.
        ...
        is_euro_office_needed = not res_ids or res_ids_wo_stream

        if is_euro_office_needed:
            ...
            for res_id in all_res_ids_wo_stream:
                try:
                    templates = self.fill_template(oo_security_token, [res_id], self.euro_office_template_id)
    ...
    def _pre_render_euro_office_pdf(self, report_ref, res_ids=None, data=None):
        if not data:
            data = {}
        ...
        return self._render_euro_office_pdf_prepare_streams(report_ref, data, res_ids=res_ids), "euro_office-pdf"

    def _render_euro_office_pdf(self, report_ref, res_ids=None, data=None):
        if not data:
            data = {}
        if isinstance(res_ids, int):
            res_ids = [res_ids]
        ...
        collected_streams, report_type = self._pre_render_euro_office_pdf(report_ref, res_ids=res_ids, data=data)
        if report_type != "euro_office-pdf":
        ...
        return pdf_content, "euro_office-pdf"
    ...
    def fill_template(self, oo_security_token, record_ids, template_id):
        ...
        docbuilder_callback_url = f"{odoo_url}euro_office/template/callback/docbuilder/fill_template?oo_security_token={oo_security_token}&record_ids={','.join([str(record) for record in record_ids])}&template_id={template_id.id}"  # noqa: E501
