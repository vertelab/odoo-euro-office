...
class ExportDataItem extends Component {
  ...
  async toggleItem(field, isUserToggle) {
    ...
    if (this.props.isFieldExpandable(id)) {
      ...
      this.env.bus.trigger("euro_office-template-create-form", field)
    }
  }
}

ExportDataItem.template = "euro_office_templates.ExportDataItem"
...
export class ExportData extends Component {
  setup() {
    ...
    useBus(this.env.bus, "euro_office-template-highlight-field", (ev) => {
  ...
  async expandToField(fieldId) {
    ...
    for (let i = 1; i < parts.length; i++) {
      ...
      if (el && !el.classList.contains("o-euro_office-list-field_expanded")) {
        el.click()
  ...
  applyHighlight(fieldId) {
    setTimeout(() => {
      const el = document.querySelector(`[data-field_id="${CSS.escape(fieldId)}"]`)
      if (el) {
        el.classList.add("o-euro_office-field-highlighted")
      }
    }, 100)
  }

  clearHighlight() {
    const highlighted = document.querySelectorAll(".o-euro_office-field-highlighted")
    highlighted.forEach((el) => el.classList.remove("o-euro_office-field-highlighted"))
  }
  ...
  async getExportedFields(model, parentParams = {}) {
    const fields = await this.orm.call("euro_office.odoo.templates", "get_fields_for_model", [
      model,
      parentParams.prefix || "",
...
ExportData.template = "euro_office_templates.ExportData"
